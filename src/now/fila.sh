...
tenho que automatizar a parte do leap, mas vou esperaar, a principio, o acesso ao leap.in que venho utilizando… mas sei que utiliza como entrada um .pdb sem hidrogenios e já extendido, eh isso? ou só um pdb sem hidrogenios?

acho que gera uma cadeia estendida a partir de um pdb sem hidrogenios!


pdb4amber with the --nohyd option to remove
all the hydrogen atoms and it was a success.

a entrada tem que ser um pdb com apenas 1 modelo!?


antes de tudo: pega o arquivo .pdb e retira todos modelos menos 1.
get_from_traj pdb modelo
depois tira os hidrogenios: pdb4amber -i 1unc.pdb -o 1unc-hydr.pdb —ypd
In general, you should use the ’-ypd’ combination of options to generate a protein-only file.

 


agora está pronto para servir de base para o parmtop gerado por uma cadeia estendida (ja venho fazendo isso no script atual) tenho que configurar o leap.in com  a sequencia.

acho que é pór ai
…
depois … preciso do fila.sh funcionando, e do setup_all_inputs

...
nrep=`wc temperatures.dat | awk '{print $2}'` #where /home/lipinski/Dropbox/gtk_remd/exemplo_analises/temperatures.dat is the temperature.dat complete path
for ext in `cat temperatures.dat` ; do
	rm gdt-total_Ttraj.$ext.dat  
	rm gdt-total_Ttraj.$ext.dat2 
	clusco_cpu -t remd_total_Ttraj.$ext.pdb -s gdt -e ../LEAP/  -o gdt-total_Ttraj.$ext.dat

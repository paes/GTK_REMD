        ...
        for section in configFile.sections():
            if section == 'commandline':
                for item in configFile.options(section):
                    args[item] = json.loads(configFile.get(section, item, 1))

                attrs['args'] = args
            elif section == 'logging':
                for item in configFile.options(section):
                    log[item] = configFile.get(section, item, 1)

                attrs['log'] = log
            elif section == 'database':
                for item in configFile.options(section):
                    database[item] = configFile.get(section, item, 1)

                attrs['database'] = database
            else:
                for item in configFile.options(section):
                    if section == 'lists' or section == 'numbers':
                        attrs[item] = eval(configFile.get(section, item, 1))
                    else:
                        attrs[item] = configFile.get(section, item, 1)
